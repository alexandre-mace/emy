        ...
        const confirmTableRows = this.props.foodstuffsToConfirm &&
            this.props.foodstuffsToConfirm['hydra:member'].map(foodstuff => (
                <tr key={foodstuff['@id']}>
                    ...
                    <td>{foodstuff.askingToOwn.firstName}</td>
                    <td><button className="form-btn" onClick={this.hasBeenTaken} value={JSON.stringify(foodstuff)} type="button" name="button">Je confirme que ce produit a été pris</button></td>
                </tr>
            ))
        ;
        return(
            <div className="col-12">
                <div className="product-confirm">
                    <h3>Ils/Elles vous attendent</h3>
                    <table>
                        <thead>
                        <tr>
                            <th>Image</th>
